...
# 初始化环境、代理和神经网络。
# 在每个时间步循环中，代理执行动作，与环境进行交互。
# 神经网络根据经验进行训练，学习如何在环境中导航。
# 显示环境，包括汽车位置、感知环境以及训练状态。
...
# MlpNn 类:
# 一个多层感知机（MLP）神经网络的实现。
# 初始化时指定输入形状，可以添加多个层，支持不同的激活函数。
# 包括前向传播、反向传播和编译（选择优化器）等方法。
# 激活函数:
# 提供了常见的激活函数，如 sigmoid、tanh、relu、leaky_relu、linear 等。
# 对应的导数函数也实现了。
# Car 类:
# 表示仿真环境中的汽车。
# 包含图像、位置、方向、速度等信息。
...
# EnvCar 类:
# 表示汽车导航的仿真环境。
# 提供 reset、step、add_map 等方法，用于重置环境、执行步骤以及添加地图信息。
# 通过 OpenCV 在窗口中显示环境。
# 在 step 方法中，处理汽车的移动、碰撞检测、感知环境以及终止条件。
# DQNAgent 类:
# 实现深度Q网络代理。
# 使用MLP神经网络作为Q函数的近似器，具有记忆重放机制。
# 包括动作选择、经验回放和保存模型的方法。
# rotate_vector 函数:
# 用于旋转2D向量的实用函数。
# car_navigation.py
# Car 类:
# 在 __init__ 中，初始化汽车的位置、图像等信息。
# record 方法用于更新汽车的状态。
# confict 方法检测是否与障碍物发生冲突。
# move 方法根据转向移动汽车。
# feel 方法模拟汽车感知环境。
# EnvCar 类:
# reset 方法用于重置环境。
# add_map 方法用于向环境中添加地图信息。
# step 方法执行一步仿真，处理用户输入、汽车移动、感知环境等。
# 在 step 方法中，根据汽车感知的状态计算奖励，并显示当前状态。
# DQNAgent 类:
# 初始化方法中定义了深度神经网络结构。
# remember 方法用于存储经验。
# act 方法根据epsilon-greedy策略选择动作。
# replay 方法执行经验回放，训练神经网络。
# rotate_vector 函数:
# 在 rotate_vector 函数中，给定角度将2D向量旋转。