...
# 神经网络类 MlpNn：
#
# __init__方法：初始化神经网络对象，设置网络的深度、学习率、优化器类型等参数。
# add_layer方法：向神经网络中添加一个隐藏层，指定该层的神经元数量和激活函数。
# forward_propagation方法：实现神经网络的前向传播过程，计算每一层的输出。
# back_propagation方法：实现神经网络的反向传播过程，根据损失函数和选择的优化器更新网络参数。
# compile方法：编译神经网络，初始化权重和偏置，并选择优化器类型。
# fit方法：训练神经网络，包括前向传播、计算损失、反向传播和参数更新。
# predict方法：对输入数据进行预测。
# show_predict2D和show_predict3D方法：用于可视化二维和三维数据的预测结果。
# initialize_optimizer方法：根据选择的优化器类型初始化优化器参数。
# summary方法：打印神经网络模型的摘要信息，包括网络层数、每层神经元数量和激活函数。
# 激活函数和损失函数：
#
# 提供了几种常用的激活函数（sigmoid、tanh、relu、leaky_relu、linear）和损失函数（square_loss）以及它们的导数函数。这些函数在神经网络的前向传播和反向传播中起到关键作用。
# 辅助函数：
#
# normalization函数：用于对数据进行标准化处理，计算数据的均值和标准差，进行数据的中心化和归一化操作。